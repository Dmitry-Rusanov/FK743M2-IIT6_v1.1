...
W9825G6KH_StatusTypeDef W9825G6KH_DumpConfig(void)
{
    ...
    printf("  Burst Type: %s\n", (DeviceConfig.BurstType == W9825G6KH_MR_BURST_TYPE_SEQUENTIAL) ? "Sequential" : "Interleaved");
    printf("  CAS Latency: %s\n", (DeviceConfig.CASLatency == W9825G6KH_MR_CAS_LATENCY_2) ? "2" : "3");
    printf("  Operating Mode: %s\n", (DeviceConfig.OperatingMode == W9825G6KH_MR_OPERATING_MODE_STANDARD) ? "Standard" : "Test");
    printf("  Write Burst Mode: %s\n", (DeviceConfig.WriteBurstMode == W9825G6KH_MR_WRITE_BURST_MODE_SINGLE) ? "Single" : "Programmed");
...
{
    if (hsdram_ptr == NULL) return W9825G6KH_ERROR;
...
{
    if (hsdram_ptr == NULL) return W9825G6KH_ERROR;
    /* Exit self refresh by sending a normal command (e.g., precharge) */
    FMC_SDRAM_CommandTypeDef Command = {0};
    Command.CommandMode = FMC_SDRAM_CMD_PALL;
    Command.CommandTarget = DeviceConfig.TargetBank;
    Command.AutoRefreshNumber = 1;
...
{
    /* Just access SDRAM to exit power down */
    if (hsdram_ptr == NULL) return W9825G6KH_ERROR;
    volatile uint16_t dummy = *(volatile uint16_t *)W9825G6KH_BANK_ADDR;
    (void)dummy;
    return W9825G6KH_OK;
}
...
{
    if (Address >= W9825G6KH_SIZE_BYTES)
        return W9825G6KH_INVALID_PARAM;
    return W9825G6KH_OK;
}
...
{
    if (hsdram_ptr == NULL) return W9825G6KH_ERROR;
    if (HAL_SDRAM_GetState(hsdram_ptr) == HAL_SDRAM_STATE_BUSY)
        return W9825G6KH_BUSY;
    return W9825G6KH_OK;
}

W9825G6KH_StatusTypeDef W9825G6KH_DebugReadModeRegister(void)
{
    /* Mode register cannot be read directly; return stored config */
    uint32_t mode_reg = DeviceConfig.BurstLength |
                        DeviceConfig.BurstType |
                        DeviceConfig.CASLatency |
                        DeviceConfig.OperatingMode |
                        DeviceConfig.WriteBurstMode;
    printf("Configured Mode Register: 0x%08lX\n", mode_reg);
    W9825G6KH_PrintModeRegisterDetails(mode_reg);
    return W9825G6KH_OK;
}

W9825G6KH_StatusTypeDef W9825G6KH_RunDiagnostics(uint32_t test_size)
{
    return W9825G6KH_MemoryTest(0, test_size);
}
